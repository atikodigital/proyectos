...
const LOCAL = path.join(__dirname, 'gastos-app', 'android', 'app', 'build', 'outputs', 'apk', 'direct', 'release', 'HashIA-direct-release-v3.94.apk');
...
conn.on('ready', () => {
  conn.sftp(async (err, sftp) => {
    ...
    sftp.fastPut(LOCAL, REMOTE, {}, async (e) => {
      ...
      await sh(`cp ${REMOTE} ${REMOTE_DIR}/HashIA-v3.94.apk`);
      const r = await sh(`ls -lh ${REMOTE} ${REMOTE_DIR}/AtikoGastos.apk ${REMOTE_DIR}/HashIA-v3.94.apk`);
      console.log('OK:\n' + r.o.trim());
      conn.end();
      console.log('LINK NUEVO (HashIA.apk): https://gastos.atikodigital.cl/panel/HashIA.apk');
      console.log('LINK V3.93: https://gastos.atikodigital.cl/panel/HashIA-v3.94.apk');
