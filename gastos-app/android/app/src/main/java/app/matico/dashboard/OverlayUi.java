...
public class OverlayUi {
    ...
    private void showPopup(String text, String pedidoId) {
        ...
        row.setOrientation(LinearLayout.HORIZONTAL);
        TextView cancel = pill("Cancelar", 0xFF444444);
        TextView send = pill("Enviar", 0xFF16A34A);
        cancel.setOnClickListener(v -> { removePopup(); showBubble(); });
        send.setOnClickListener(v -> doSend(edit.getText().toString(), pedidoId));
        LinearLayout.LayoutParams l = new LinearLayout.LayoutParams(0, LinearLayout.LayoutParams.WRAP_CONTENT, 1f);
        l.rightMargin = dp(6);
        LinearLayout.LayoutParams r = new LinearLayout.LayoutParams(0, LinearLayout.LayoutParams.WRAP_CONTENT, 1f);
        r.leftMargin = dp(6);
    ...
    private void doSend(String texto, String pedidoId) {
        removePopup();
        toast("Enviando…");
        new Thread(() -> {
            boolean ok = service.enviarEnChatActual(texto);
            if (ok && pedidoId != null && !pedidoId.isEmpty()) {
                try { AtikoBackend.markSent(service, pedidoId); } catch (Exception ignored) {}
            }
            ui.post(() -> { toast(ok ? "✅ Pedido enviado" : "No pude escribir en el chat."); showBubble(); });
        }).start();
    }
