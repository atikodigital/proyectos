...
/**
 * VARAS — esfera dorada audio-reactiva (HUD), tema ORO #C9A24B.
 * Props: { state, audioLevel, onTap }
 * state: 'off' | 'connecting' | 'live' | 'listening' | 'speaking' | 'error'
 */
export default function VarasOrb({ state = 'off', audioLevel = 0, onTap }) {
  // En jest/jsdom saltamos el loop rAF para tests deterministas.
  const isTest =
  ...
  // Color por estado — tema ORO
  let color;
  if (error) color = '#ff5a5a';
  else if (state === 'connecting') color = '#dcc483';
  else if (speaking) color = '#e6c55f';
  else if (listening) color = '#C9A24B';
  else color = '#b08b3a';
    ...
    >
      <div className="relative flex items-center justify-center">
        ...
        <svg width="170" height="170" viewBox="0 0 300 300" style={{ overflow: 'visible' }}>
          ...
          <g transform={`rotate(${tick * 18} 150 150)`} filter="url(#varas-glow)">
            ...
            {Array.from({ length: 36 }).map((_, i) => {
              ...
              return (
                <line key={i} x1={150 + Math.cos(a) * r1} y1={150 + Math.sin(a) * r1} x2={150 + Math.cos(a) * r2} y2={150 + Math.sin(a) * r2}
                  stroke={color} strokeWidth={i % 3 === 0 ? 1.4 : 0.8} opacity={i % 3 === 0 ? (idle ? 0.35 : 0.7) : idle ? 0.18 : 0.35} />
              );
            })}
          </g>

          <g transform={`rotate(${-tick * 32} 150 150)`} filter="url(#varas-glow)">
          ...
          {(speaking || listening) &&
            Array.from({ length: 64 }).map((_, i) => {
              ...
              return (
                <line key={i} x1={150 + Math.cos(a) * r1} y1={150 + Math.sin(a) * r1} x2={150 + Math.cos(a) * r2} y2={150 + Math.sin(a) * r2}
                  stroke={color} strokeWidth="1.5" opacity={0.5 + seed * 0.5} strokeLinecap="round" />
              );
            })}
          ...
          <circle cx="150" cy="150" r={20 + lvl * 12 + (speaking ? Math.sin(tick * 9) * 1.5 : Math.sin(tick * 2) * 0.8)} fill="url(#varas-core)" filter="url(#varas-glow)" />
      ...
      <span className="font-black text-xs tracking-widest mt-1" style={{ color: '#C9A24B', letterSpacing: '0.15em' }}>
        VARAS
      </span>
    </div>
  );
