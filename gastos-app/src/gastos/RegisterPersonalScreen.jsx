...
export default function RegisterPersonalScreen({ onRegistered, onBackToLogin }) {
  ...
  async function submit(e) {
    ...
    try {
      ...
      if (msg === 'email_en_uso') setError(t('regp.err_email_uso'));
      else setError(t('regp.err_generico'));
    } finally {
      setLoading(false);
    }
  }
  ...
  if (incomeStep) {
    return (
      <div className="min-h-screen flex flex-col justify-center gap-4 p-6 max-w-sm mx-auto">
        <h1 className="text-2xl font-black" style={{ color: '#C9A24B' }}>{t('regp.bienvenido')}</h1>
        <p className="text-sm opacity-70">{t('regp.income_q')}</p>
        <label className="text-sm" htmlFor="rp-income">{t('regp.income_label')}</label>
  ...
  return (
    <form onSubmit={submit} className="min-h-screen flex flex-col justify-center gap-4 p-6 max-w-sm mx-auto">
      ...
      <p className="text-sm opacity-70">{t('regp.sub2')}</p>
      ...
      <label className="text-sm" htmlFor="rp-email">{t('reg.correo')}</label>
      ...
      <label className="text-sm" htmlFor="rp-pass">{t('reg.password_min6')}</label>
      ...
      <button type="submit" disabled={loading} className="rounded-xl font-black py-3 text-black disabled:opacity-50" style={{ background: '#C9A24B' }}>
        {loading ? t('reg.creando') : t('regp.crear')}
      </button>
