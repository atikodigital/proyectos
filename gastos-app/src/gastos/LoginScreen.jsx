...
export default function LoginScreen({ onLoggedIn }) {
  ...
  return (
    <div className="min-h-screen flex flex-col justify-center gap-5 p-6 max-w-sm mx-auto">
      ...
      <div>
        <p className="text-sm font-bold text-center mb-3 opacity-80">{t('login.como_usar')}</p>
        <div className="grid grid-cols-2 gap-3">
          <button type="button" onClick={() => setMode('register')}
            aria-label={t('login.negocio')}
            ...
            <span className="font-black text-sm">{t('login.negocio')}</span>
            <span className="text-[11px] opacity-60 text-center leading-tight">{t('login.negocio_sub')}</span>
          </button>
          <button type="button" onClick={() => setMode('personal')}
            aria-label={t('login.personal')}
            className="rounded-2xl border-2 p-4 flex flex-col items-center gap-1 active:scale-95 transition"
            style={{ borderColor: '#C9A24B' }}>
            <span className="text-3xl" aria-hidden="true">👤</span>
            <span className="font-black text-sm">{t('login.personal')}</span>
            <span className="text-[11px] opacity-60 text-center leading-tight">{t('login.personal_sub')}</span>
          </button>
        </div>
      </div>
      ...
      <div className="flex items-center gap-3 opacity-50">
        ...
        <span className="text-xs whitespace-nowrap">{t('login.ya_tienes')}</span>
        <div className="h-px flex-1" style={{ background: 'currentColor' }} />
      </div>

      {/* Login para usuarios existentes */}
      <form onSubmit={submit} className="flex flex-col gap-4">
        <label className="text-sm" htmlFor="usuario">{t('login.usuario')}</label>
        ...
        <label className="text-sm" htmlFor="password">{t('login.password')}</label>
        ...
        <button type="button" onClick={() => { setOlvido(true); setOlvidoMsg(''); setOlvidoIdent(usuario); }}
          className="text-xs underline opacity-60 mx-auto mt-1">{t('login.olvidaste')}</button>
      </form>
      ...
      {googleDisponible() && (
        <div className="flex flex-col gap-3">
          <div className="flex items-center gap-3 opacity-50">
            <div className="h-px flex-1" style={{ background: 'currentColor' }} />
            <span className="text-xs whitespace-nowrap">{t('login.o_entra_con')}</span>
            <div className="h-px flex-1" style={{ background: 'currentColor' }} />
          </div>
      ...
      {olvido && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.75)', zIndex: 60, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
          <div style={{ background: '#15171a', border: '1px solid #343535', borderRadius: 16, padding: 20, width: '100%', maxWidth: 360 }}>
            <h3 style={{ margin: '0 0 6px', fontWeight: 900, color: '#C9A24B' }}>🔑 {t('olvido.titulo')}</h3>
            ...
            <div style={{ display: 'flex', gap: 8, marginTop: 14 }}>
              <button type="button" onClick={() => setOlvido(false)} style={{ flex: 1, padding: 10, borderRadius: 10, border: '1px solid #343535', background: 'transparent', color: '#d0c6ab', fontWeight: 700 }}>{t('olvido.cancelar')}</button>
              {olvidoMsg[0] !== '✅' && (
                <button type="button" onClick={enviarOlvido} disabled={olvidoEnviando}
                  ...
                  {olvidoEnviando ? t('olvido.enviando') : t('olvido.enviar')}
                </button>
              )}
            </div>
          </div>
